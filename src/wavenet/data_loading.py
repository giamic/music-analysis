...
def _parse_function(proto):
    f = {
        ...
        "x": tf.FixedLenSequenceFeature([], tf.float32, default_value=0, allow_missing=True)
    }
...
def create_tfrecords_iterator(input_path, batch_size, shuffle_buffer):
    ...
    if shuffle_buffer is None:
        dataset = dataset.map(_parse_function, num_parallel_calls=16).repeat().batch(batch_size).prefetch(batch_size)
    else:
        dataset = dataset.map(_parse_function, num_parallel_calls=16).shuffle(shuffle_buffer).repeat().batch(batch_size).prefetch(batch_size)
